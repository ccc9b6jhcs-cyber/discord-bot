...
async function getAssetOwners(assetId, limit = 50) {
  // Format: { data: [{ id, owner: { id, type }, created }] } — no username in response
  const owners = [];
  let cursor = '';
  ...
  while (owners.length < limit) {
    ...
    for (const entry of (data.data || [])) {
      // entry = { id: UAID, owner: { id: RobloxUserId, type: "User" }, created, serialNumber }
      // entry.owner.id is the actual Roblox user ID
      // entry.id is the UserAsset ID (UAID) — NOT the user ID
      const uid = entry.owner?.id;
      if (uid && entry.owner?.type === 'User') {
        owners.push({ userId: String(uid), username: null });
      }
      if (owners.length >= limit) break;
      // Log first entry so we can verify structure
      if (owners.length === 1) console.log('First owner entry:', JSON.stringify(entry));
    }
  ...
  // Batch fetch usernames — POST up to 100 IDs at once
  const ids = owners.map(o => o.userId);
  try {
    const batchRes = await fetch('https://users.roblox.com/v1/users', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify({ userIds: ids.slice(0, 100).map(Number), excludeBannedUsers: false })
    });
    console.log('Username batch status:', batchRes.status);
    const batchText = await batchRes.text();
    console.log('Username batch response:', batchText.slice(0, 300));
    if (batchRes.ok) {
      const batchData = JSON.parse(batchText);
      const nameMap = {};
      for (const u of (batchData.data || [])) nameMap[String(u.id)] = u.name;
      console.log('Name map size:', Object.keys(nameMap).length, 'sample:', JSON.stringify(Object.entries(nameMap).slice(0,3)));
      for (const o of owners) o.username = nameMap[o.userId] || o.username || 'Unknown';
    }
  } catch (e) { console.error('Username batch failed:', e.message); }
