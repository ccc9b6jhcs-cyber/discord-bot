...
async function getAssetOwners(assetId, limit = 50) {
  ...
  // Batch fetch usernames from Roblox users API
  ...
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      const nameMap = {};
      for (const u of (data.data || [])) nameMap[String(u.id)] = u.name;
      for (const o of owners) o.username = nameMap[o.userId] || 'Unknown';
    }
  } catch (e) { console.error('Username lookup failed:', e.message); }
