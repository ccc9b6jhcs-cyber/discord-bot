...
// ─── Safely extract item IDs from offering field (handles any format) ─────────
function extractOfferingIds(offering) {
  ...
  if (Array.isArray(offering)) {
    return offering.map(item => {
      ...
      if (typeof item === 'object' && item !== null) return String(item.id || item.assetId || Object.values(item)[0]);
      return String(item);
    }).filter(Boolean);
  }

  // Object/map format: { "itemId": [...], ... }
...
async function registerCommands() {
  ...
  console.log('✅ /lookfor registered globally with DM support');
}
