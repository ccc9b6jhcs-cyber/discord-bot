...
async function fetchWithRetry(url, options = {}, retries = 3) {
  ...
  throw new Error('Roblox owners API 429 — rate limited after retries');
}
...
async function getAssetOwners(assetId, limit = 50) {
  ...
  while (owners.length < limit) {
    const url = `https://inventory.roblox.com/v2/assets/${assetId}/owners?limit=10&sortOrder=Asc${cursor ? `&cursor=${cursor}` : ''}`;
